...
const Recentcalls = ({ getrecentcalls, state }) => {
  ...
  const loadrecentcallsdata = (pageno) => {
    ...
    getrecentcalls(params).then((response) => {
      console.log(state.contactbooks.managedlinfo);
      debugger;
      setCallshistory(response.data.resultset);
      setPagecount(Math.ceil(response.data.totalrows / constants.PAGE_SIZE));
      setProgress(false);
      console.log(state.common.recentcalls);
    });
  };
          ...
          {callshistory.map((tdata, i) => (
            <tr className="rowstyle">
              <td align="center" width="5%">
                <img
                  src={
                    tdata.conftype === "instant"
                      ? imgcall
                      : tdata.conftype === "schedule"
                      ? imgclock
                      : tdata.conftype === "sms"
                      ? imgmsg
                      : imgmic
              ...
              <td align="left" width="50%">
                ...
                <span>&nbsp;Lasted&nbsp;{tdata.duration}</span>
                <br />
                <span className="callendreason">{tdata.confdescription}</span>
                <span className="conftime">&nbsp;&nbsp; {tdata.conftime}</span>
              </td>
              <td align="right" width="12%">
                {tdata.conftype === "voiceblast" ? (
                  <span>
                    <span className="conftime">Not Listened:</span>
                    <span className="confmems">
                      {tdata.totalmembers -
                        (tdata.delevered + tdata.listened + tdata.partial)}
                    </span>
                  </span>
                ) : (
                  ""
