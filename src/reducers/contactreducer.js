...
var contactsdefault = {
  ...
  selectedtab: "fav",
  editgroup: {
    data: { resultset: [], nextpage: 0, groupcode: 0, groupname: "" },
  },
  pageresultset: [],
...
const contactReducer = (state = contactsdefault, action) => {
  switch (action.type) {
    case "allcontacts":
      var checkedstate = new Array(action.payload.data.resultset.length).fill(
        false
      );
      ...
      for (let i = 0; i < state.newgroup.selectedcontacts.length; i++) {
        for (let j = 0; j < action.payload.data.resultset.length; j++) {
          if (
            state.newgroup.selectedcontacts[i].userid ===
            action.payload.data.resultset[j].userid
          ) {
    ...
    case "allgroups":
      var groupcheckedstate = new Array(
        action.payload.data.resultset.length
      ).fill(false);

      for (let i = 0; i < state.newgroup.selectedgroups.length; i++) {
        for (let j = 0; j < action.payload.data.resultset.length; j++) {
          if (
            state.newgroup.selectedgroups[i].userid ===
            action.payload.data.resultset[j].userid
          ) {
      ...
      return {
        ...
        pagecount: Math.ceil(
          action.payload.data.totalrows / constants.PAGE_SIZE
        ),
    ...
    case "editgroup":
      var excludemembersstate = new Array(
        action.payload.data.resultset.length
      ).fill(false);
      ...
      for (let i = 0; i < state.newgroup.exculdedcontacts.length; i++) {
        for (let j = 0; j < action.payload.data.resultset.length; j++) {
          if (
            state.newgroup.exculdedcontacts[i].userid ===
              action.payload.data.resultset[j].userid &&
            action.reqmsg.groupcode ===
              state.newgroup.exculdedcontacts[i].groupcode
          ) {
